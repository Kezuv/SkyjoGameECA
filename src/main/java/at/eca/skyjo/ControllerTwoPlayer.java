...
public class ControllerTwoPlayer implements Initializable {
        ...
        button.setOnMouseClicked(event1 -> {
                if (event1.getButton() == MouseButton.PRIMARY) {
                    if (gameTwoPlayer.permissionCheck(player, i, cardNumber)) {
                        if (!player.getCard(cardNumber).isFaceUp()) {
                            if (!gameTwoPlayer.isCanPickUp()) {
                                if (player == player1) {
                                    player1Table(cardNumber, button);
                                    gameTwoPlayer.setMovesLeft(gameTwoPlayer.getMovesLeft() - 1);
                        ...
                        if (gameTwoPlayer.permissionCheck(player, i, cardNumber) && !gameTwoPlayer.isFirstRound()) {
        ...
        deckImgFaceDown.setOnMouseClicked(e -> {
            if (gameTwoPlayer.isCanPickUp()) {
                if (e.getButton() == MouseButton.PRIMARY) {
                    ...
                    gameTwoPlayer.setCanPickUp(false);
    ...
    public void nextPlayer(ActionEvent actionEvent) throws IOException {
        ...
        if (gameTwoPlayer.getMovesLeft() == 0) {
            switch (gameTwoPlayer.gamePlay(gameTwoPlayer.getPlayers((gameTwoPlayer.getIsPlaying() - 1)), gameTwoPlayer.getIsPlaying())) {
                case 1 -> {
                    refresh();
